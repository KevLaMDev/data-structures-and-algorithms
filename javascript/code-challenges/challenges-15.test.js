...
const screenForNames = (arr) => {
  let regex = /^[MD][rs][.][ ][a-zA-Z]|[M][r][s][.][ ][a-zA-Z]/
  return arr.filter(str => regex.test(str))
};
// input: arr of str
// output: new arr of filtered strs
  // use filter to iterate over arr of strs and return new arr
    // use regex 
...
const sortBy = (property, arr) => {
  if (typeof(arr[0][property]) === 'number') {
    return arr.sort((objA, objB) => objA[property] - objB[property])
  } else if (typeof(arr[0][property]) === 'string') {
    return arr.sort((objA, objB) => (objA[property] > objB[property]) ? 1 : -1)
  };
};
