...
function transformToLis(obj){
  let propsArr = Object.entries(obj);
  return propsArr.map(arr => `<li>${arr[0]}: ${arr[1]}</li>`)
};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 2
...
const count = (target, input) => {
  if (!input[0]) return 0;
  let arr = input.reduce((a,b) => a.concat(b));
  let filteredArr = arr.filter(el => el === target);
...
const totalSum = (input) => input.reduce((a,b) => a.concat(b)).reduce((acc, currVal) => acc += currVal);
...
const divisibleByFiveTwoToThePower = (input) => {
  let filteredArrs = input.map(arr => arr.filter(el => typeof(el) === 'number' && el % 5 === 0));
  return filteredArrs.map(arr => arr.map(el => 2 ** el));
};
// input: array of arrays
// output: return arr of arr with respective els as 2 raised to the power of them
  // use map on arr of arrs
    // use filter to filter out els that are not nums or not divisble by 5
  // use map on filtered arr and raise 2 to the power of each el
/* ------------------------------------------------------------------------------------------------
CHALLENGE 5
...
let findMaleAndFemale = (data) => {
  ...
  for (let i =0; i < data.length; i++) {
    if (data[i].gender === 'male' || data[i].gender === 'female') str += `${data[i].name} and `;
  };
  str = str.split(' ')
...
let findShortest = (data) => {
  let heightsArr = data.map(obj => obj.height);
  let minHeight = heightsArr.reduce((acc, currVal) => {
    if (acc < currVal) acc = currVal;
    return acc;
  }, 0);
  let shortestChar = data.filter(obj => obj.height === minHeight);
  return shortestChar[0].name;
};
// input: arr of objs
// output: return character shortest in height
  // use map to get arr of heights
  // use reduce to find smallest height val
  // use filter to get obj with height prop === smallest height val
  // return obj 
