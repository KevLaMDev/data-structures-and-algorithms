...
// output: return the same arr with names sorted alpha, capital letters come before lowercase
// arr.sort(callback on docs)
...
const sortByPrice = (arr) => {
  return arr.sort((objA, objB) => objA.price - objB.price);
};
...
const sortNumbersByLength = (arr) => {
  ...
  let sortedStr = arr.sort((numA, numB) => {
    return numA.length - numB.length;
  });
  console.log(sortedStr)
  return sortedStr.map(str => {
...
const sortPeople = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    lastNamesArr.push(arr[i].lastName);
  };
  lastNamesArr = lastNamesArr.sort();
  let newArr = [];
  for (let i = 0; i < arr.length; i++) {
    for (let j = 0; j < arr.length; j++) {
      if (lastNamesArr[i] === arr[j].lastName) newArr.push(arr[j])
    }
  }
  return newArr;
};
...
const sortMeetingsByDay = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (i === 0) {
      for (let j = 0; j < arr.length; j++) {
        if (arr[j].dayOfWeek === 'Monday') newArr.push(arr[j])
      }
    }
    else if (i === 1) {
      for (let j = 0; j < arr.length; j++) {
        if (arr[j].dayOfWeek === 'Tuesday') newArr.push(arr[j])
      }
    }
    else if (i === 2) {
      for (let j = 0; j < arr.length; j++) {
        if (arr[j].dayOfWeek === 'Wednesday') newArr.push(arr[j])
      }
    }
    else if (i === 3) {
      for (let j = 0; j < arr.length; j++) {
        if (arr[j].dayOfWeek === 'Thursday') newArr.push(arr[j])
      }
    }
    else if (i === 4) {
      for (let j = 0; j < arr.length; j++) {
        if (arr[j].dayOfWeek === 'Friday') newArr.push(arr[j])
      }
    }
  }
  return newArr;
...
const sortSchedule = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (i === 0) {
      for (let j = 0; j < arr.length; j++) {
        if (arr[j].dayOfWeek === 'Monday') {
          newArr.push(arr[j]);
          counter++;
          if (counter > 1) {
            if (arr[j].start < sameDay.start) {
              newArr.splice(indexOf(sameDay), 1, arr[j]);
              newArr.pop;
              newArr.push(sameDay);
            } else if (arr[j].start === sameDay.start) {
              if (arr[j].end - arr[j].start < sameDay.end - sameDay.start) {
                newArr.splice(indexOf(sameDay), 1, arr[j]);
                newArr.pop;
                newArr.push(sameDay);
              }
