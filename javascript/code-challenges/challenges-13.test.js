...
const findHappiness = (arr) => {
  let splitArr = arr.map(el => el.split('')); // arr of arrs
  let newArr = [];
...
const allHappy = (arr) => {
  let splitArr = arr.map(el => el.split('')); // arr of arr
  console.log(splitArr)
  for (let i = 0; i < splitArr.length; i++) {
    let smilePresent = false;
    for (let j = 0; j < splitArr[i].length; j++) {
      if (arr[i][j] === ':') {
        if (arr[i][j+1] === ')') {
          smilePresent = true;
    ...
    if (!smilePresent) return false;
  };
  return true;
};
// input: arr of str
// output: boolean if all strings contain :)
  // splitArr = arr.map(str => str.split(''));
    // splitArr is now arr of arrs
  // nested for loop to iterate thru arrs
    // 
/* ------------------------------------------------------------------------------------------------
